...
onMounted(async () => {
  try {
    ...
    deudor1.value = response.data;
  } catch (error) {
    console.error(error);
  }
});

const deudor = ref<Deudor | null>(usePage().props.deudor as Deudor || null);

//Formulario para agregar el curp y rfc en caso de que el formulario de agregar deudor se haya omitido
const form = useForm({
    curp: deudor.value ? deudor.value.curp : null || '',
    rfc: deudor.value ? deudor.value.rfc : null || '',
});

const submit = () => {
    form.patch(route('deudores.agregarInfoDeudor', id), {
        ...
        <div class="text-center mx-auto px-10 py-5 sm:px-6 lg:max-w-6xl lg:px-8 bg-gray-100">
            <div class="container max-w-screen-lg mx-auto">
                <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6 border-2">
                    ...
                    <center><h1 class="font-semibold text-xl text-gray-800 leading-tight mx-auto text-center">{{ deudor1[0]?.nombre }} {{ deudor1[0]?.apellidoPaterno }} {{ deudor1[0]?.apellidoMaterno }}</h1></center>
                    <br>
                    <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-2">
                        ...
                        <div class="lg:col-span-2">
                            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
                                ...
                                <div v-if="((deudor1[0]?.curp === null || deudor1[0]?.curp === '' || deudor1[0]?.curp != null) && (deudor1[0]?.rfc === null || deudor1[0]?.rfc === '')) || ((deudor1[0]?.curp === null || deudor1[0]?.curp === '') && (deudor1[0]?.rfc === null || deudor1[0]?.rfc === ''  || deudor1[0]?.rfc != null))" class="md:col-span-6">
